...
function SettingsView({ data, repository, actorUid, onRefresh }: { data: AppData; repository: BudgetRepository; actorUid: string; onRefresh: () => Promise<void> }) {
  ...
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  function editItem(itemKind: Exclude<typeof kind, "rules">, item: Account | Category | SettlementPool | Project | RecurringTemplate) { setKind(itemKind); setEditingId(item.id); setName(item.name); setDetail("notes" in item ? item.notes ?? "" : "description" in item ? item.description : "description" in item ? item.description : ""); }
  async function archiveItem(itemKind: Exclude<typeof kind, "rules">, item: Account | Category | SettlementPool | Project | RecurringTemplate) { const timestamp = new Date().toISOString(); if (itemKind === "account") await repository.saveAccount(householdId, { ...(item as Account), active: false, updatedAt: timestamp }); else if (itemKind === "category") await repository.saveCategory(householdId, { ...(item as Category), active: false }); else if (itemKind === "pool") await repository.saveSettlementPool(householdId, { ...(item as SettlementPool), active: false }); else if (itemKind === "project") await repository.saveProject(householdId, { ...(item as Project), status: "archived" }); else await repository.saveRecurringTemplate(householdId, { ...(item as RecurringTemplate), active: false, updatedAt: timestamp }); await onRefresh(); }
